...
class Affiliate::VoucherListsControllerTest < ActionController::TestCase
  fixtures :all
  ...
  context "on get to show with a closed voucher list" do
    setup do
      ...
      @prev_list.vouchers.available.each { |v| v.claim!(Iou.paid.first) }
    ...
    should "have not be downloaded" do
      assert !@list.downloaded
      assert !@prev_list.downloaded
    end
  end
